...
export const createStripeAccount = functions.https.onCall(async (data, context) => {
    ...
    if (userSnap.exists && userSnap.data()?.stripeConnectId) {
        return { accountId: userSnap.data()?.stripeConnectId };
    }

    try {
        const account = await stripe.accounts.create({
            type: "express",
            country: "US", // Change to match your region (e.g., 'GB' for UK)
...
export const getStripeOnboardingLink = functions.https.onCall(async (data, context) => {
    ...
    const accountId = userSnap.data()?.stripeConnectId;
    ...
    try {
        const accountLink = await stripe.accountLinks.create({
            account: accountId,
            refresh_url: "https://YOUR_APP_URL/dashboard?stripe_refresh=true", // Replace with your app URL
            return_url: "https://YOUR_APP_URL/dashboard?stripe_return=true",   // Replace with your app URL
...
export const handleStripeConnectWebhook = functions.https.onRequest(async (req, res) => {
    ...
    const endpointSecret = functions.config().stripe?.webhook_secret; // Set via config
 ...
 */
export const authorizeJobPayment = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (jobData?.supplierId !== context.auth.uid) {
        ...
        if (!haulerSnap.exists || !haulerSnap.data()?.isStripeVerified) {
        ...
        const amountInCents = Math.round(jobData?.price * 100);
        // Platform fee (e.g., 10%)
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            ...
            customer: jobData?.stripeCustomerId, // If you have it, else omit
            confirm: true, // Attempt to authorize immediately
            capture_method: "manual", // <--- THE KEY: Holds funds, doesn't charge yet
            metadata: {
                jobId: jobId,
 ...
 */
export const captureAndPayout = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        try {
            ...
            const haulerStripeId = haulerDoc.data()?.stripeConnectId; // "acct_..."
 ...
 */
export const checkJobExpirations = functions.pubsub
    ...
    .onRun(async (context) => {
         ...
         * AUDIT LOGGING
         * ------------------------------------------------------------------
         * Trigger: Firestore onUpdate (transportJobs/{jobId})
         * Logic: Records critical changes to 'audit_logs' collection.
         */
        export const logJobChanges = functions.firestore
            .document("transportJobs/{jobId}")
            .onUpdate(async (change, context) => {
                const before = change.before.data();
                const after = change.after.data();
                ...
                // Fields to monitor
                const monitoredFields = ["status", "paymentStatus", "price", "haulerOrgId", "assignedDriverId", "pickupTime"];
                const changes: any[] = [];
                ...
                // Attempt to identify actor (Best Effort)
                // 1. If 'assignedBy' changed, that's the actor for assignment.
                // 2. If 'lastModifiedBy' exists (future proof), usage that.
                // 3. Infer from logic (e.g. status assigned -> in_transit = hauler).
                let actorId = after.lastModifiedBy || "unknown"; // Ideally frontend sends this

                if (before.assignedBy !== after.assignedBy) {
                    actorId = after.assignedBy;
                } else if (after.status === "in_transit" && before.status === "assigned") {
                    actorId = after.assignedDriverId || after.haulerId;
                }
                ...
                const auditRecord = {
                    ...
                    actorId: actorId,
                    changes: changes,
                    // Store a snapshot for redundancy if needed
                    // snapshot: after 
                };

                await db.collection("audit_logs").add(auditRecord);
                console.log(`Audit log created for Job ${jobId}`, changes);
                return null;
            });

        /**
         * ------------------------------------------------------------------
         * ROLE MANAGEMENT (Multi-Tenancy)
         * ------------------------------------------------------------------
         * Callable Function: createOrgUser
         * Triggered by: Org Admin/Owner to create sub-users (Driver, Dispatcher, Clerk)
         */
        export const createOrgUser = functions.https.onCall(async (data, context) => {
            ...
            // 2. Validate Input
            const validRoles = ["driver", "dispatcher", "clerk", "audit_viewer", "admin"];
            if (!email || !password || !validRoles.includes(role)) {
                throw new functions.https.HttpsError("invalid-argument", "Valid email, password, and role required.");
            }

            try {
                // 3. Verify Requester Permissions (Must be Admin/Owner of the Org)
                const requesterSnap = await db.collection("users").doc(requesterUid).get();
                const requesterData = requesterSnap.data();

                if (!requesterData?.organizationId) {
                    throw new functions.https.HttpsError("failed-precondition", "You do not belong to an organization.");
                }

                // strict check: Only Owner or Admin can create users
                if (!['owner', 'admin'].includes(requesterData.role)) {
                    throw new functions.https.HttpsError("permission-denied", "Only Admins or Owners can create team members.");
                }

                const orgId = requesterData.organizationId;
                ...
                // 5. Create User Profile in Firestore (Linked to Org)
